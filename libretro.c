...
void retro_run(void)
{
   ...
   if (l && !last_l && audio_streams_num > 0)
   {
      ...
      char msg[256];
      snprintf(msg, sizeof(msg), "Audio File #%d.", audio_streams_ptr);
   ...
   else if (r && !last_r && subtitle_streams_num > 0)
   {
      ...
      char msg[256];
      snprintf(msg, sizeof(msg), "Untertitel #%d.", subtitle_streams_ptr);
      environ_cb(RETRO_ENVIRONMENT_SET_MESSAGE, &(struct retro_message) { .msg = msg, .frames = 180 });
   }
   ...
   if (seek_frames)
   {
      ...
      char msg[256];
      snprintf(msg, sizeof(msg), "Suche: %u s.", (unsigned)seek_time);
      environ_cb(RETRO_ENVIRONMENT_SET_MESSAGE, &(struct retro_message) { .msg = msg, .frames = 180 });

      if (seek_frames < 0)
      {
         if (log_cb)
            log_cb(RETRO_LOG_INFO, "Resette PTS.\n");
         frames[0].pts = 0.0;
         frames[1].pts = 0.0;
      }
...
static bool open_codec(AVCodecContext **ctx, unsigned index)
{
   ...
   {
      if (log_cb)
         log_cb(RETRO_LOG_ERROR, "Konnte keinen geeigneten Decoder finden ... \n");
      return false;
   }
      ...
      SwrContext *swr)
{
   ...
   for (;;)
   {
      ...
      while (!decode_thread_dead && fifo_write_avail(audio_decode_fifo) < required_buffer)
      {
         ...
         {
            if (log_cb)
               log_cb(RETRO_LOG_ERROR, "Thread: Audio Deadlock erkannt ...\n");
            fifo_clear(audio_decode_fifo);
            break;
         }
      }
...
static void decode_thread_seek(double time)
{
   ...
   if (ret < 0 && log_cb)
      log_cb(RETRO_LOG_ERROR, "av_seek_frame() Fehler.\n");
   ...
   while (!decode_thread_dead)
   {
      ...
      else if (pkt.stream_index == subtitle_stream)
      {
         ...
         while (!finished)
         {
            ...
            {
               log_cb(RETRO_LOG_ERROR, "Untertitel decoding Fehlgeschlagen.\n");
               break;
            }
         }
...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   if (!environ_cb(RETRO_ENVIRONMENT_SET_PIXEL_FORMAT, &fmt))
      LOG_ERR_GOTO("Kann Pixel Format nicht einstellen.", error);

   if (avformat_open_input(&fctx, info->path, NULL, NULL) < 0)
      LOG_ERR_GOTO("Kann nicht Geöffnet werden.", error);

   if (avformat_find_stream_info(fctx, NULL) < 0)
      LOG_ERR_GOTO("Stream info Fehlt.", error);

   av_dump_format(fctx, 0, info->path, 0);

   if (!open_codecs())
      LOG_ERR_GOTO("Finde den richtigen Code nicht.", error);

   if (!init_media_info())
      LOG_ERR_GOTO("Medien Info Fehler.", error);
   ...
   if (video_stream >= 0)
   {
      ...
      if (!environ_cb(RETRO_ENVIRONMENT_SET_HW_RENDER, &hw_render))
         LOG_ERR_GOTO("Render kann nicht initialisiert werden.", error);
#endif
   }
   if (audio_streams_num > 0)
